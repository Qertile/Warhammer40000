...
- 新增 `parseTerminologyToList()` 解析器，將 `terminology.md` 轉換為純粹的「英文 → 中文」清單，去除標題與括號說明。
- 解決 AI 將原本的 Markdown 說明文字（如括號內的解釋）誤認為譯名一部分的問題。
- 強化 System Instruction 中的翻譯指令，明確標示「唯一合法」與「禁止自創」規則。
- 多變體支援：如 `Necrons / Necron` 會自動展開為獨立的對應規則。

### 🔄 伺服器行程清理與重啟
- 手動清理佔用 Port 3000 的舊行程，確保新開發代碼（解析邏輯）正確生效。
- 統一開發環境於 Port 3000 運行。
